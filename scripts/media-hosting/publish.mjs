...
export async function publishReferenceImage(rootArg, input, fetchImpl = fetch) {
  rejectSecrets(input)
  if (input.confirmed !== true) throw new Error('公开上传参考图前必须 confirmed=true')
  ...
  if (!/^v\d{3}$/.test(input.version_id || '')) throw new Error('version_id 必须是 v001 格式')
  const root = await realpath(resolve(rootArg))
  const lockKey = createHash('sha256').update(`${input.service}:${input.asset_key}:${input.version_id}`).digest('hex')
  return withFileLock(resolve(root, '.short-drama/locks', `publish-${lockKey}`), async () => {
    ...
    const path = await realpath(resolve(root, version.localPath))
    ...
    const reusable = (await listReferenceUploads(root, { service: input.service, asset_key: asset.key, version_id: version.id, state: 'active' }))
      .find((item) => item.sha256 === version.sha256 && item.usage_scope === input.usage_scope)
    ...
    const receipt = { version: 1, id: `upload-${randomUUID()}`, service: input.service, media_type: 'image', asset_key: asset.key, version_id: version.id, local_path: version.localPath, sha256: version.sha256, size_bytes: version.sizeBytes, url, expires_in: input.expires_in, usage_scope: input.usage_scope, confirmations: { rights: true, public_exposure: true, terms_of_use: true }, created_at: createdAt.toISOString(), expires_at: new Date(createdAt.getTime() + EXPIRY_MS[input.expires_in]).toISOString(), permanent: false }
    if (await sha256(path) !== version.sha256 || (await stat(path)).size !== version.sizeBytes) throw new Error('上传期间本地图片发生变化，拒绝登记公开链接')
