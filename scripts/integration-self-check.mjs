...
async function main() {
  ...
  try {
    ...
    const generatedMarketReportRef = JSON.parse(run('project-store.mjs', 'market-report-ref', root, marketReport.report_id, 'hyp-original-hook'))
    const marketInspiration = {
      schema_version: 'market-inspiration.v1',
      report_ref: generatedMarketReportRef,
    ...
    await writeFile(resolve(timingDirectory, 'v001.json'), `${JSON.stringify({ episode_key: 'ep-001', source_asset: timingSource, method: 'manual-direction', reviewed: true, language: 'zh-CN', lines: [{ line_index: 1, speaker: 'A', text: '别绕弯子', start_ms: 0, end_ms: 5000, pauses: [{ start_ms: 2500, end_ms: 2700 }], review_evidence: { speaker_checked: true, text_checked: true, visible_mouth_checked: true, notes: '集成夹具逐帧核对' }, words: [{ text: '别绕弯子', start_ms: 500, end_ms: 2500 }], evidence: '集成夹具人工起止与停顿证据' }] }, null, 2)}\n`)
