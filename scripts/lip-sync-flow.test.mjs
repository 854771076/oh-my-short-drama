...
async function fixture({ presentation = 'visible-dialogue', delivery = 'post_dub' } = {}) {
  ...
  const video = resolve(root, 'assets/videos/shot-ep001-001/v001.mp4'), audio = resolve(root, 'assets/audio/audio-ep001-line-001/v001.wav')
  ...
  await putAsset(root, { key: 'audio-ep001-line-001', type: 'audio', name: '对白 1' })
  await addAssetVersion(root, 'audio-ep001-line-001', { id: 'v001', localPath: 'assets/audio/audio-ep001-line-001/v001.wav', provenance: { origin: 'imported', created_by: 'user', provider: null, model_or_workflow: null, task_id: null, prompt_document: null, source_assets: [], parameters: {} } })
...
test('MuseTalk 输出经请求快照登记为未选中的变换候选', async () => {
  ...
  try {
    ...
    assert.deepEqual(candidate.provenance.source_assets, [{ key: 'shot-ep001-001', version_id: 'v001' }, { key: 'audio-ep001-line-001', version_id: 'v001' }])
  } finally {
