...
async function main() {
  ...
  if (command !== 'audit' || !rootArg || !/^ep-\d{3}$/.test(episodeKey) || !/^\d+$/.test(shotRaw) || !assetKey || !/^v\d{3}$/.test(versionId) || confirmation !== '--confirmed') throw new Error('用法：native-audio-audit.mjs audit <项目> <ep-001> <镜号> <视频资产 key> <版本> --confirmed')
  const root = resolve(rootArg), shotNumber = Number(shotRaw)
