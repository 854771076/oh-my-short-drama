...
export async function validateGenerationDocumentReference(root, type, target, reference, args, provider, model, requireCurrent = true) {
  ...
  if (type === 'audio') {
    ...
    if (!timingSource || timingSource.line_index !== line.line_index) throw new Error('音频生成必须绑定当前 selected speech-timing 与来源 SHA')
    await selectedSourceSpeechTiming(root, timingSource)
