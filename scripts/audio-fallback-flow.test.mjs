...
test('兜底预检保存异常、区间和混音来源且不产生付费任务', async () => {
  ...
  try {
    ...
    const plan = { episode_key: 'ep-001', approved: true, unresolved: [], lines: [{ line_index: 1, speaker: 'narrator', content: '风暴终于来了。', matched_shot: { shot_number: 1 }, delivery_mode: 'native', presentation: 'narration', fallback_mode: 'cinematic-tts', performance: { tone_arc: '平静转警觉', emotion_beats: ['低声', '停顿', '收束'], pace: '中慢', breath_and_pause: '转折停半拍', distance_and_space: '近讲' } }], voice_bindings: [{ speaker: 'narrator', provider: 'bailian', model: 'cosyvoice-v3.5-plus', voice_id: 'narrator-voice' }] }
