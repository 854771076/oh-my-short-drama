...
const characterAppeal = await readFile(resolve(root, 'scripts/character-appeal.mjs'), 'utf8')
const characterAppealReview = await readFile(resolve(root, 'scripts/character-appeal-review.mjs'), 'utf8')
const referenceBindings = await readFile(resolve(root, 'scripts/reference-bindings.mjs'), 'utf8')
const workflowStages = await readFile(resolve(root, 'scripts/workflow-stages.mjs'), 'utf8')
...
for (const token of ['native-first', 'native', 'post_dub', 'external_audio', 'visible-dialogue', 'offscreen-dialogue', 'narration', 'cinematic-tts', 'native_audio_exception', 'provider-no-native-audio', 'voice-identity-drift', 'speech-intelligibility-failed', 'narration-performance-failed', 'audio-sync-failed', 'native-ambience-failed']) if (!audioPlanContract.includes(token)) failures.push(`原生优先声音合同缺少：${token}`)
for (const token of ['speech_intelligibility', 'speaker_identity', 'narration_performance', 'ambience_action_sync', 'lip_sync', 'technical_audio', 'undeclared_music']) if (!nativeAudioAudit.includes(token)) failures.push(`原生音频七维审核缺少：${token}`)
...
const sourceTimedDubbing = await Promise.all(['scripts/speech-timing.mjs', 'scripts/audio-plan-contract.mjs', 'scripts/dubbing-compiler.mjs', 'scripts/dubbing-fit.mjs', 'scripts/dubbing-performance-review.mjs', 'scripts/subtitles-from-audio.mjs'].map((path) => readFile(resolve(root, path), 'utf8'))).then((items) => items.join('\n'))
for (const token of ['0.9', '0.8', 'max_paid_generations', '0.97', '1.03', 'semantic_integrity', 'speaker_identity', 'emotion_arc', 'intensity_and_subtext', 'emphasis_pause_breath', 'timing_fit', 'picture_interaction', 'technical_audio', 'audio_binding']) if (!sourceTimedDubbing.includes(token)) failures.push(`源时间线配音门禁缺少：${token}`)
for (const token of ['先从当前 selected 原声资产建立', '最多三轮', '八维审核', '最终 selected 音频', '禁止自动追加第四次']) if (!audioFlowGuide.includes(token)) failures.push(`源时间线配音 Skill 强制措辞缺少：${token}`)
