...
for (const token of ['assetRoot', 'enforceGenerationStage', 'inspectStage', 'validateVideoPrompts', '视频生成必须引用有效的 episode/version/shot', 'selected 提示词版本', '实际视频参数与提示词文档', 'project.json 已确认配置一致', 'maxItems: 12', 'list_media_hosts', 'list_reference_uploads', 'publish_reference_image', 'reserveTask']) if (!generationMcp.includes(token)) failures.push(`生成入口边界校验缺少：${token}`)
