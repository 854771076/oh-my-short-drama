...
for (const token of ['selectedDocumentRecord', 'missingStoryboardReviews', '分镜图八维审计', '制作计划与分镜镜号不一致', '视频提示词来源版本不是当前选版', "stage === 'delivery'", "editing', episode", "delivery', episode", 'evidenceSha256', 'invalidatedAt', 'art-style.json', 'staleVersionIds']) if (!workflowGates.includes(token)) failures.push(`工作流硬门禁缺少：${token}`)
