...
test('原声时间到配音选版、字幕和失效传播完整闭环', async () => {
  ...
  try {
    ...
    const versions = []
    for (const [id, bytes] of [['v001', 'attempt-one'], ['v002', 'attempt-two-good'], ['v003', 'attempt-three-better']]) {
      await writeFile(resolve(audioDir, `${id}.wav`), bytes)
    ...
    await writeFile(resolve(videoDir, 'v001.mp4'), 'base-video')
    ...
    const [subtitle] = buildSubtitlesFromAudio({ audio: { asset_key: 'audio-ep001-line-001', version_id: 'v002', sha256: versions[1].sha256 }, timing: { version_id: 'v001', lines: timingDocument.lines }, contracts: [{ line_index: 1, content: '别回头', speaker: '林晚', dubbing_contract_version: 'v001', dubbing_contract: contract }], fps: 24, timeline_end_ms: 1000 })

    await writeFile(resolve(videoDir, 'v002.mp4'), 'lip-video')
    const audioBinding = subtitle.audio_binding
    await addAssetVersion(root, 'shot-ep001-001', { id: 'v002', localPath: 'assets/videos/shot-ep001-001/v002.mp4', provenance: transformed([{ key: 'shot-ep001-001', version_id: 'v001' }, { key: 'audio-ep001-line-001', version_id: 'v002' }], { operation: 'lip-sync', parameters: { audio_binding: audioBinding }, source_sha256: sha('base-video') }) })
    ...
    const acceptance = await runDubbingLiveAcceptance(root, 'ep-001', 1, { original: 'shot-ep001-001@v002', dub: 'audio-ep001-line-001@v002', review: resolve(root, '.short-drama/dubbing-reviews.json') })
