...
test('原声时间到配音选版、字幕和失效传播完整闭环', async () => {
  ...
  try {
    ...
    const sourceDir = resolve(root, 'assets/videos/shot-ep001-source')
    await mkdir(sourceDir, { recursive: true })
    media(resolve(sourceDir, 'v001.mp4'), 'video')
    await putAsset(root, { key: 'shot-ep001-source', type: 'video', name: '原声来源镜头' })
    await addAssetVersion(root, 'shot-ep001-source', { id: 'v001', localPath: 'assets/videos/shot-ep001-source/v001.mp4', provenance: imported() })
    await selectAssetVersion(root, 'shot-ep001-source', 'v001')
    ...
    await addAssetVersion(root, 'shot-ep001-001', { id: 'v002', localPath: 'assets/videos/shot-ep001-001/v002.mp4', provenance: transformed([{ key: 'shot-ep001-001', version_id: 'v001' }, { key: 'shot-ep001-source', version_id: 'v001' }, { key: 'audio-ep001-line-001', version_id: 'v002' }], { operation: 'lip-sync', range: { start_ms: 100, end_ms: 900 }, parameters: { audio_binding: audioBinding }, source_sha256: sourceSha }) })
    ...
    const immutableReport = await readFile(reportPath, 'utf8')
    const repeated = await runDubbingLiveAcceptance(root, 'ep-001', 1, { original: 'shot-ep001-source@v001', dub: 'audio-ep001-line-001@v002', review: resolve(root, '.short-drama/dubbing-reviews.json') })
    assert.equal(repeated.output, reportPath)
    assert.equal(repeated.report.created_at, acceptance.report.created_at)
    assert.equal(await readFile(reportPath, 'utf8'), immutableReport)
