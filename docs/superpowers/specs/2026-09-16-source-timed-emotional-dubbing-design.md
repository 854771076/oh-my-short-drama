...
2. **参数重生**：根据实际发声时长调整自然范围内的语速、停顿和指令；计算结果钳制到合同的 `0.85–1.15` 后必须实际生成该边界值，不能因为原始计算值越界就在第二轮提前阻断，也不能使用接口允许的极端边界。
...
比较前必须先把音频内相对时间换算到剧集时间线；`actual_start` 取首词起点，`actual_end` 取末词终点。每个中间词的边界误差仍完整写入验收报告，用于停连、节奏和异常定位，但自然韵律造成的词内时长重分配不单独触发硬失败。字幕阅读余量不参与上述发声边界误差计算。
