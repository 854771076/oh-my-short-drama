...
RunningHub 既支持用户自己的通用工作流，也内置迁移自现项目的 MiniMax H3 专用工作流：

- 环境变量：`RUNNINGHUB_API_KEY`、可选 `RUNNINGHUB_BASE_URL`，以及通用工作流的 `RUNNINGHUB_IMAGE_WORKFLOW_ID`、`RUNNINGHUB_VIDEO_WORKFLOW_ID`、`RUNNINGHUB_AUDIO_WORKFLOW_ID`。H3 默认平台工作流 ID 为 `2086743729407733762`，可用 `RUNNINGHUB_H3_WORKFLOW_ID` 覆盖。
