---
name: plan-shot-continuity
description: 依据已批准分镜、导演本和制作计划建立逐镜空间与状态连续性合同；用于规划轴线、站位、持物、出入画和上一镜尾帧承接，不生成媒体。
---

# 规划跨镜连续性

读取当前 selected 且已批准的 `storyboard`、`director-book` 和 `production-plan`，并使用 `assets/prompts/continuity_plan.{zh,en}.txt` 生成一份 `continuity-plan`。只采用这些文档、已选资产和已通过审核的白模导演合同中的证据；不得推断未声明的道具、持物关系、入画边、出画边或精确米制坐标。
