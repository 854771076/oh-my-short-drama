---
name: publish-drama-references
description: 将短剧项目中已选本地参考图片临时发布为公网 URL。用于 Comfly 等只接受公开 HTTPS 素材的 Provider；不用于最终资产存储或长期图床。
---

# 临时发布短剧参考图

仅当目标 Provider 不接受本地文件、确实需要公网 HTTPS URL 时使用。先调用 `list_media_hosts` 查看当前能力，再调用 `list_reference_uploads` 检查同一资产版本是否已有有效收据；默认可选 `litterbox`，匿名免费，无 API Key，过期时间只能是 `1h`、`12h`、`24h` 或 `72h`，不是永久图床。
...
服务合同与限制见 [Litterbox](references/litterbox.md)。
