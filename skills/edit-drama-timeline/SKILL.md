...
需要裁剪、换声、稳定、降噪、调色、局部修复、补帧、对口型或超分时，先切换到 `transform-drama-media` 执行统一媒体操作；时间线不得直接引用刚生成的候选。候选必须完成对应专项观察、结构化 QC 与完整观看，并由 `review_media_operation` 批准成为 selected 后才可导入。对口型专项审核未通过时，`dialogue_sync` 不得标记为 `lip-synced`；超分专项审核未验证分辨率、时长帧率、音轨、脸手文字和运动伪影时，不得把高分辨率文件当成正式镜头。
