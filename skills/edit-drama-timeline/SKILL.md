...
字幕必须来自实际音轨转写并人工复核，不能用提示词冒充。模型提示词刻意禁字、但剧情依赖手机界面、倒计时、消息、地点或时间信息时，必须把这些内容作为确定性的 Remotion 图形层纳入时间线与完整审片，不能只在制作计划备注“后期制作”。时间线按 [剪辑合同](../../references/editing-workflow.md) 分开保存 `segments[]`、`audio_tracks[]`、Remotion Caption `subtitles[]`、`labels[]` 和 `mix`；J/L-cut 通过独立音轨时间范围表达。切点服从叙事、动作、视线和声音；优先硬切、动作切、视线切、J/L-cut 或构图匹配，短淡化只用于明确的时间、地点或情绪变化。Remotion composition 的 `durationInFrames` 必须由时间线实际总时长计算，不能硬编码 60 秒；项目单集时长只是目标，默认约 ±15% 的自然偏差可接受，优先保留动作完成、对白阅读和结尾呼吸。每次渲染候选前执行 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/freeze-edit-candidate.mjs" create <项目> <episode-key> <candidate-key> <timeline> <Remotion目录>`，冻结时间线、渲染器源码和全部媒体哈希；候选 key 不得覆盖复用。先渲染低码率审片版，用 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/media-tools.mjs" qc <文件>` 检查媒体参数、黑帧、冻结、静音与响度事件；完整观看和听完后用 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/editing-store.mjs" put-review <项目> <episode-key> <审片.json>` 保存六项结论，通过后才渲染交付版。
