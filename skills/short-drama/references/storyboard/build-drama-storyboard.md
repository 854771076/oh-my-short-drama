...
分镜选定且制作计划完成后，必须交给 `plan-shot-continuity` 建立独立的 `camera_setup_id`、`start_state`、`end_state` 和 `transition_link` 合同，再进入视频提示词编译。分镜只描述本镜事实，不得提前伪造上一镜尾帧资产或用模糊的“保持连续”代替状态字段。分镜选定后，若后续 `plan-drama-production` 或 `write-drama-video-prompts` 因缺构图证据、素材缺口或镜头需要拆并而产生 `unresolved`，必须退回 `revise-drama-storyboards` 生成新的不可变分镜版本，不覆盖旧版，再重新走制作计划与视频提示词。
