...
复刻镜头强烈推荐在正式视频生成前先制作深度视频。每镜必须在 `video_strategy.depth_reference` 显式记录 `{mode:"generate"|"waived",provider,workflow_id,source_asset_key,source_version_id,expected_output_asset_key,reason}`：有可用原视频运动、空间或运镜证据时默认 `generate`，固定推荐 `provider=runninghub`、`workflow_id=2098674379113979905`；只有没有有效时间参考、目标模型不能接收视频参考或用户明确选择创意偏离时才可 `waived`，并填写具体原因。不得静默省略该字段或把人物参考图当作深度视频替代品。
