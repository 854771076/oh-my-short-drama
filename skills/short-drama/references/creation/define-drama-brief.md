...
合并询问缺失且会改变创作结果的最少问题，不替用户选择题材或平台。由 Codex 使用 `../../assets/modules/define-drama-brief/prompts/drama_brief.{zh,en}.txt` 直接输出结构化 `brief.json`，至少包含：一句话目标、题材/受众/基调、核心冲突、交付语言、画幅、集数、单集目标时长、内容分级、已有素材、明确禁区；确认后用 `project-store.mjs put-document <项目目录> brief <文件>` 保存，并用 `update-project` 同步 `format`、`languages` 和 `creative` 中对应的项目级事实。`episode_duration_seconds` 表示策划目标而非必须精确命中的硬截止；除非平台明确限长，后续优先保证动作完成、对白可读和结尾呼吸。
