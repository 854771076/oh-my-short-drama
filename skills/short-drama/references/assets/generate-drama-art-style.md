...
读取 brief、bible、用户参考和项目当前 `creative.art_style`。未提出自定义画风时保留默认 `system-realistic`；提出明确画风时，由 Codex 使用 `../../assets/modules/generate-drama-art-style/prompts/config_generation.{zh,en}.txt` 生成 prompt、description 和 visualBible，不调用文本 Provider。
