...
开始视频制作前先检查 `video_strategy.depth_reference`：`mode=generate` 时必须已有对应 source 生成的 selected 深度视频，并以 `type=video, role=depth_reference` 进入本镜提示词引用；没有深度选版就停止，不得静默降级。`mode=waived` 时必须保留具体 reason；缺字段同样停止并退回制作计划。
