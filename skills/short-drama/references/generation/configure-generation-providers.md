...
- RunningHub：环境变量 `RUNNINGHUB_API_KEY`，可选基址和通用图片/视频/音频工作流 ID；内置图片 `krea2-normal-v1`，可用 `RUNNINGHUB_KREA2_WORKFLOW_ID` 覆盖；内置 H3 可用 `RUNNINGHUB_H3_WORKFLOW_ID` 覆盖平台工作流 ID。复刻流程强烈推荐先用 RunningHub 工作流 `2098674379113979905` 把原镜头制成深度视频，再把该深度视频作为 H3 或 StarRouter Seedance 的普通视频参考；它是结构/运动/运镜参考，不宣称原生 Depth/ControlNet。视频变换目录固定提供 `seedvr2.5-video-upscale`，绑定工作流 `2099866760106491906` 的官方真实节点映射，不读取自定义超分工作流变量，也不列入普通视频生成目录。协议见 [RunningHub 合同](../../../../references/runninghub-provider.md)。
