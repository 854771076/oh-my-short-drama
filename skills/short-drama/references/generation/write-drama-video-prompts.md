...
先读取每镜 `video_strategy.depth_reference`。`mode=generate` 时必须找到由所列原视频版本生成且已选版的深度视频，把它作为独立 `type=video, role=depth_reference` 引用加入，并明确它只约束空间结构、人物运动和运镜；人物参考图仍独立约束身份与外观。深度视频缺失或版本不符时写入 errors，不得略过后继续编译。`mode=waived` 必须带具体 reason；字段缺失时退回制作计划。
