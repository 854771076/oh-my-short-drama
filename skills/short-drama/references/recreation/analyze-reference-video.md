...
读取 `../../assets/modules/analyze-reference-video/prompts/reference_video_analysis.{zh,en}.txt` 的 `reference_video_analysis` Codex 合同，用 prepared 清单、项目配置、关键帧观察和转写生成 `.short-drama/reference-video-analysis.json`。每项结构判断必须回链 `evidence[].id`；原片事实、机制推断和创作建议分开，不能把标题、画面猜测或不完整 ASR 写成确定事实。
