...
每个阶段开始前，先运行 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/module-runs.mjs" required <项目目录> <阶段>`，逐个完整读取返回 Skill 的 `SKILL.md` 并按其合同产出；不能只参考总入口或自行模仿产物。每个 Skill 完成后运行 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/module-runs.mjs" record <项目目录> <阶段> <skill> <项目内证据路径...>`；同一阶段已有各 Skill 证据、只是配置或提示词变更导致凭证失效时，可运行 `module-runs.mjs record-stage <项目目录> <阶段>` 一次重算证据哈希和提示词绑定。`workflow.mjs <check|advance>` 会同时检查产物和 Skill 执行凭证；缺少任一项都不得推进。
