...
- 本地参考短视频拆解与声明式复刻模板 → `analyze-reference-video`、`use-hypit-video`、`design-video-recreation`；只在 `workflow.type=viral-recreation` 时由 analysis 阶段条件加载，Hypit 固定用于准备后、正式分析前的中间理解
- Hypit 完整视频编排、SVML/SVS/SVRun、Studio、Build 与变体 → `use-hypit-video`；非复刻项目仅在用户明确要求时使用，首次使用按需安装官方 Skill 和可执行程序
