...
画风库由系统画风库 `references/art-styles.json` 与统一工作区根目录下的 `art-styles.custom.json`（或 `SHORT_DRAMA_CUSTOM_ART_STYLES_FILE`）合并组成。选择或确认自定义画风后，必须保存完整 style 对象到自定义库并按 `id` upsert；系统画风只读，不得被自定义对象覆盖。自定义画风必须包含 `id`、`name`、`description`、`prompt` 和完整 v1 `visualBible`，未确认或不完整的候选不得登记。
