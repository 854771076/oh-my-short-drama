...
出图前必须对人物档案执行 `assertCharacterReadyForVisuals`：未知年龄不得生成视觉资产；成年人物必须保留已批准的 `adult-charisma`、专属妆发、服装标识和全部记忆锚点；儿童只允许 `child-cuteness`，禁止成人妆容、成人化服装、性感姿态、身体曲线强调和性化镜头语言。魅力设计服从时代、职业、阶层、伤病和剧情状态，不能靠无据裸露或模板化网红脸替代人物设计。

对所有无依赖形象全量提交 `generate_image`；RunningHub 同一 API Key 最多 2 路并发，其余请求由适配器排队；同一人物的派生造型只在基础形象选版后进入下一并发层。`prompt_document` 必须引用该人物所在的当前 `asset-plan`：`{kind:"asset-plan",episode_key,version_id,asset_key}`；实际 prompt 必须等于 `character_asset_sheet` 的已留痕输出。参考图仅传该人物已选主形象，避免串脸。提交前确认模型、数量、尺寸、参考路径和费用。检查脸型、发型、体型、服装版本、时代与标志物；每个候选都用 `asset-ledger.mjs fetch|decode` 保存到本地人物目录并写入相同制作文档引用。新生成候选禁止直接 `asset-ledger select`，必须先按 `review-drama-shots` 的人物专项合同完整查看原图，再由 `review-ledger.mjs review-character` 原子审核和选版。
