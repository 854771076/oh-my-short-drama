...
所有媒体资产使用稳定 key、类型、输入指纹、候选版本和 `selectedVersionId`。key 必须使用 `char-`、`scene-`、`prop-`、`board-`、`shot-`、`audio-` 或 `other-` 前缀，版本使用 `v001`。先用 `node scripts/asset-ledger.mjs put <项目目录> <资产.json>` 建档；本地结果用 `import`，Provider 返回的 HTTPS URL 用 `fetch`，base64 结果文件用 `decode`。三者都会把内容保存到项目并记录本地相对路径、文件大小、SHA-256 和固定 provenance。
