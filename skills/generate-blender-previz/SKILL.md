---
name: generate-blender-previz
description: 使用 Blender 生成可选为正式分镜媒介的白模预演视频，用于复杂调度、轴线、动作和专业运镜验证；不用于最终成片。
---
...
兵器用 `attach_to` 绑定 UE Mannequin 的 `left_hand` 或 `right_hand`，并以 `contact_checks` 检查 `tip`/`origin` 到对手骨骼或另一件兵器的距离。术法预设只表达发射、轨迹、碰撞和消散，不把辉光或粒子数量当成动作成立；施法者起手、目标反应、冲击点和镜尾结果仍须分别可读。预设字段与适用边界见 `direct-blender-previz/references/weapons-vfx.md`。
