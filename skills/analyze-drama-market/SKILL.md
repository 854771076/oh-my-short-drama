...
先确认用户提供的工作区存在且可访问，再执行 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/market-research.mjs" list <workspace>`，核对最新快照、最新报告和报告覆盖范围。必须先读取本 Skill 的 [方法说明](references/methodology.md)，再解读任何指标。
