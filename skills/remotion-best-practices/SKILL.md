...
字幕必须从实际成片音轨转写并人工复核，内部统一使用 Remotion `Caption` JSON：`text`、`startMs`、`endMs`、`timestampMs`、`confidence`，可附 `speaker` 和样式；同时输出烧录字幕、UTF-8 SRT 和 ASS。高级字幕采用“说话人小号信息 + 正文 + 细色线/轻量半透明渐变承托”的层次，避免厚重黑色圆角贴纸；承托不能遮脸、手、关键道具或画内文字，必须在缩小预览和安全区内复核。时间线中的语义换行用 `whiteSpace: pre-line` 保留，不能把长句完全交给浏览器自动折行而产生孤字；中文常规阅读速度约 6–12 字/秒，短时回声只能等义压缩，不能为了塞入时间码改写剧情事实。每条独立音轨记录 selected 资产版本、源/时间线范围、角色和 `volume_envelope`；混音固定 `target_lufs` 与 `true_peak_dbtp`。H3 原生整轨只允许轻量、可逆的增益、EQ、降噪、压缩、限幅与短交叉淡化，不得用外部 TTS/BGM 覆盖失败。
