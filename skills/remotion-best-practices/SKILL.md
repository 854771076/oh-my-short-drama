...
没有 Remotion 工程时，用 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/render-edit.mjs" scaffold <项目目录> <episode-key>` 从插件内置模板（`assets/remotion-template/`）生成确定性工程到 `editing/<episode-key>/remotion/`：自动把 selected 媒体复制进 `public/media/`、生成带 `file` 字段与 `subtitle_layout` 的 `public/timeline.json` 和可打包的 `src/timeline.generated.ts`；已有工程直接复用。禁止手搓 `npx create-video` 或绕过内置模板散落硬编码。素材放入工程 `public/`，用 `staticFile()` 引用；视频用 `OffthreadVideo`、音频用 `Audio`（均从 `remotion` 引入，时间裁切用 `trimBefore`/`trimAfter` 帧字段），图片使用 `<Img>`。依赖版本由模板 `package.json` 固定，`render-edit.mjs render` 首次自动 `npm install`，不要在工程内 `npx remotion add` 漂移版本。composition 的 `durationInFrames` 必须从 timeline 实际尾点计算，不能把“一分钟短剧”硬编码为 60 秒；目标时长默认允许约 ±15% 自然浮动，平台明确限长除外。
...
预览用 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/render-edit.mjs" render <项目目录> <episode-key> --preview`（底层 `remotion studio --no-open`），审片版与交付版统一用 `node "${CODEX_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-.}}/scripts/render-edit.mjs" render <项目目录> <episode-key> [--out <path>]`（底层 `npx remotion render` 出片，默认 `delivery/<episode-key>/final.mp4`）；QC 响度不达标时用 `render-edit.mjs loudness` 做 loudnorm 后处理。完整观看和听完、保存六项审片结论后才渲染交付版。成片目标与转场、响度、角标、画面连续性和交付清单以 `editing-workflow.md` 为准。
